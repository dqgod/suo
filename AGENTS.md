...
- The built-in `>` prefix is an explicit high-risk terminal action, never an immediate query side effect. Typing only creates a result; activation must consume a one-time backend token bound to its query epoch and terminal-config snapshot, recheck the current configuration and run without elevation. Windows obtains System32 from the native system API and opens the selected PowerShell/CMD console; macOS opens a protected temporary `.command` through the configured terminal application. Do not trust `PATH`/`SystemRoot`, send an executable command as a WebView action payload, or capture terminal output in the launcher.
...
- Bundled `examples/` scripts are read-only initialization templates. On startup, copy missing templates into the platform-default app config directory's `scripts/` folder with create-new semantics; never overwrite an existing user script. Relative `scripts/...` paths resolve from that fixed user directory before bundled compatibility fallbacks, even when `config.json` is relocated.
...
- The active `config.json` may be relocated, but discovery must always start from the platform-default app config directory and its versioned `config-location.json` pointer. Write and validate the target config before atomically switching the pointer, reject occupied custom destinations, and keep the previous config as a recovery copy.
...
- Dock visibility is a macOS Settings-window presentation preference. When enabled, show the Dock icon only while Settings is open and hide it again when Settings closes; when disabled, keep it hidden at all times without dismissing the open Settings window. Keep the typed field cross-platform for portable configuration, but isolate native behavior behind the macOS adapter. On Windows, keep the shortcut-driven launcher out of the taskbar, keep Settings in the taskbar, and preserve the independent coloured tray icon.
- Tauri's `app.macOSPrivateApi` must remain enabled while macOS uses transparent undecorated windows; without it `transparent: true` is ignored and the rounded CSS shell exposes white native-window corners. This private transparency path is incompatible with Mac App Store distribution, which remains outside the current scope.
...
- Settings-window component typography, including command summaries and Appearance-editor guidance, must derive from the active settings theme's base font-size token with readable floors. Launcher preview typography remains scoped to the launcher theme being previewed.
- Launcher result icons follow a stable type contract: applications discovered in the runtime catalog may request native icons from their validated local paths, directories use the built-in folder icon, and other files use the built-in file icon. Script and web-search commands may embed one optional local PNG/JPEG/WebP data URL no larger than 256 KB or 512 px per side; validate and bounded-decode it in both the selection UI and Rust save path, and never load a remote icon URL. Their optional empty-argument hint is presentation copy only and must not impose a parameter schema or disable a script that validly accepts zero arguments.
- `suo-json-v1` is UTF-8. Plain text output may use a configured encoding and is capped independently for stdout/stderr.
- Successful scripts use a case-sensitive typed line protocol: legacy `SUO_RESULT:` and `SUO_RESULT:text:` select text lines, `SUO_RESULT:image:` accepts one bounded local PNG/JPEG/WebP data URL (or raw PNG Base64), and `SUO_RESULT:qrcode:` asks the native core to render one local QR PNG. QR content is limited to 500 UTF-8 bytes, declares UTF-8 ECI 26, and preserves at least 4 px per module in the generated PNG. The launcher displays all result images as complete, height-aware thumbnails capped at 240 px. A result cannot mix text and image types. If no prefix exists, preserve legacy full stdout. Never apply this parser to stderr or a subsequently executed result-shell command.
- Script result images must be decoded and validated in Rust before serialization: at most 512 KB decoded, 1024 px per side, PNG/JPEG/WebP only, no remote URLs or SVG. The webview repeats the allow-list before assigning an image source.
- Configuration v16 migrates only the built-in `timestamp-example` command from `examples/timestamp.py` to `scripts/timestamp.py`; preserve every custom or absolute script path unchanged. Configuration v17 adds the `qr-example` only when the `qr` keyword/alias and ID are unused, and never overwrites an existing user script.
- Configuration v18 adds `launcher.terminal`. Older configurations enable the built-in `>` command with PowerShell/Terminal defaults unless `>` is already owned by a translation, script or web-search keyword/alias; in that collision case preserve the user command and migrate with the built-in disabled.
