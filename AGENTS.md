...
- Configuration v16 migrates only the built-in `timestamp-example` command from `examples/timestamp.py` to `scripts/timestamp.py`; preserve every custom or absolute script path unchanged.
