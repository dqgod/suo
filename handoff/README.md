...
最后更新：2026-09-12。[`v0.1.3`](https://github.com/dqgod/suo/releases/tag/v0.1.3) 仍是当前最新 Pre-release（配置 v17）；`dev` 正在开发 0.1.4 / 配置 v18，新增 `>` 内置终端命令和设置页组件字号联动。macOS arm64 已发布原生全屏 Space 覆盖修复，Windows x64 必须从同一不可变 tag 重新构建并追加资产。已经完成的构建、实机和旧版本流水均已移入 [`archive/`](archive/README.md)，当前平台文件只保留未完成项。

## 当前状态

| 范围 | 状态 | 下一步 |
| --- | --- | --- |
| `dev` 0.1.4 / 配置 v18 | **代码候选，待双平台实机验证** | 验证 `>` 只在 Enter 后执行、平台终端选择、一次性授权、v17 冲突迁移，以及设置基础字号对命令摘要/外观编辑器说明的联动；详见两个平台文件。 |
...
- [`WINDOWS.md`](WINDOWS.md)：`dev` 0.1.4 新能力实机清单，以及 `v0.1.3` Windows x64 构建、真实回归和 Release 追加清单。
- [`MACOS.md`](MACOS.md)：`dev` 0.1.4 新能力实机清单，以及 `v0.1.3` macOS 仍未关闭的多显示器、v17 真实迁移和原生集成验证。
