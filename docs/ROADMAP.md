...
本文件集中维护动态开发状态和后续计划。详细平台执行记录与仍未关闭的验证项见 [`handoff/`](../handoff/README.md)，稳定的产品范围和验收标准见 [`PRODUCT_REQUIREMENTS.md`](./PRODUCT_REQUIREMENTS.md)。
...
- `> <命令>` 提供可关闭的内置终端入口：输入阶段只生成结果，按 Enter 后才以当前用户权限打开可见终端；Windows 可选 PowerShell/CMD，macOS 可配置支持 `.command` 的终端应用；动作使用一次性后端授权，配置 v18 遇到已有 `>` 用户命令时保留用户项并默认关闭内置入口；
...
- 设置皮肤基础字号现在同时驱动命令摘要和外观编辑器说明文字；默认说明文字具有 12 px 可读下限，元信息保留防拥挤上限，右侧主题预览仍使用被预览皮肤自身字号；
...
- `dev` 当前开发候选版本为 0.1.4、配置协议 v18；`>` 终端入口和设置页组件字号联动仍需 Windows/macOS 安装版实机验证，不能当作已发布能力。
- [`v0.1.3`](https://github.com/dqgod/suo/releases/tag/v0.1.3) 是当前最新 Pre-release，已发布 macOS Apple Silicon arm64 ZIP 与 SHA-256；自动化门禁、最终 ZIP 解包冷启动和单显示器原生全屏 Space 实机验证已完成。
- Windows x64 仍需从同一不可变 tag 重新构建、验证并向该 Release 追加安装包。
- 当前版本仍未使用 Developer ID 签名或公证。未完成项见 [`handoff/MACOS.md`](../handoff/MACOS.md) 和 [`handoff/WINDOWS.md`](../handoff/WINDOWS.md)，完整证据与旧版本流水见 [`handoff/archive/`](../handoff/archive/README.md)。
