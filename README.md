...
- 提供午夜、纸张、森林三套内置皮肤，以及颜色、材质、布局和平台覆盖可调的自定义皮肤；支持严格校验的 `suo-theme-v1` JSON 导入导出和本地背景图；
- 应用搜索结果异步加载系统原生图标，并限制并发、缓存与可访问路径；文件夹使用统一文件夹图标，其余文件使用统一文件图标；
